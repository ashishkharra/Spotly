...
import { registerValidationSchema, signInValidationSchema, googleSignInSchema } from '../validations/auth.js';
...
const SignInUp = () => {
  ...
  const handleOAuthData = async (authData) => {
    setShowToast(false);

    try {
      await googleSignInSchema.validate(authData, { abortEarly: false });

      const res = await sendRequest("POST", "/OAuth", authData);
      ...
      if (res?.success) {
        ...
        setToastMessage(res?.message === 'Server error' ?  res?.results : res?.message);
        setToastType("error");
        setShowToast(true);
      }
    } catch (err) {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-gray-900 to-gray-700 overflow-hidden p-2">

      <div className="z-50 absolute">{showToast && <Toast message={toastMessage} type={toastType} />}</div>

      <div className={`p-5 relative w-full md:w-[800px] max-w-full ${isSignUp ? 'min-h-[600px]' : 'min-h-[500px]'} bg-white rounded-[30px] shadow-lg overflow-hidden transition-all duration-500`}>

        {/* Sign In Form */}
        <div className={`absolute top-0 left-0 h-full mt-20 w-full md:mt-0 lg:mt-0 md:w-1/2 flex flex-col items-center justify-center px-4 md:px-10 transition-all duration-500 ${isSignUp ? "md:translate-x-full opacity-0 z-10" : "opacity-100 z-30"}`}>
          ...
          >
            {({ isSubmitting, values, handleChange }) => (
              <Form className="w-full">
                ...
                <button
                  type="submit"
                  className="bg-indigo-900 w-full text-white font-bold text-sm uppercase px-6 py-2 rounded-lg mt-4 flex justify-center items-center gap-2 cursor-pointer"
          ...
          <OAuth para={isSignUp} onOAuthData={handleOAuthData} role={role} />

        </div>

        {/* Sign Up Form */}
        <div className={`absolute top-0 left-0 h-1/2 mt-[220px] sm:mt-[250px] sm:py-20 md:mt-[130px] lg:mt-[130px] w-full md:w-1/2 flex flex-col items-center justify-center px-4 md:px-10 transition-all duration-500 ${isSignUp ? "opacity-100 z-30" : "md:translate-x-full opacity-0 z-10"}`}>
          <h2 className="text-2xl font-bold">Sign Up</h2>
          <p className="text-sm mb-1">or use your email account</p>
          ...
          <Formik
            ...
            onSubmit={async (values, { setSubmitting }) => {
              ...
              try {
                ...
                const res = await sendRequest("POST", "/register", registerData);

                if (res?.success) {
                  setTimeout(() => {
                    setToastMessage(res?.message || "Registered successfully!");
                    setToastType("success");
                    setShowToast(true);
                  }, 10);
        ...
        {/* Overlay Container */}
        <div className={`absolute top-0 left-0 md:left-auto md:right-0 w-full md:w-1/2 h-1/3 md:h-full overflow-hidden transition-all duration-500 z-40 ${isSignUp ? "-translate-y-0 md:translate-y-0 md:translate-x-0" : "md:translate-x-0"}`}>
          <div className="h-fit py-7 sm:h-full bg-gradient-to-r from-orange-600 to-orange-800 text-white flex flex-col items-center justify-center px-8 text-center rounded-[10px]">
            <h2 className="text-2xl font-bold">{isSignUp ? "Welcome Back!" : "Hello, Friend!"}</h2>
            <p className="text-sm my-1">{isSignUp ? "Already have an account? Login here." : "Don't have an account? Sign up now!"}</p>
            <button onClick={toggleForm} className="bg-transparent border border-white text-white font-bold text-sm uppercase px-4 py-1 rounded-lg mt-4 z-50">
